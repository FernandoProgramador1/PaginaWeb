...
class Valores extends Conectar {
    private $table;
    private $view;
    private $id;
    private $lastid;
    ...
    public function getWhereVision($value)  {
        ...
        $sql = "SELECT * FROM {$this->view} WHERE IdValores='{$this->val}' ";
        // echo $sql;
        $result = $this->db->query($sql);
        while($row = $result->fetch_assoc())   {
            $this->field[] = $row;
    ...
    public function insertValores($desc) {
    ...
    public function updateValores($value,$desc)  {
    ...
    public function deleteValores($value)  {
        $this->id = $value;
