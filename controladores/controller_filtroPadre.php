...
class FiltroPadre extends Conectar {
    private $table;
    private $view;
    private $id;
    private $lastid;
    ...
    public function getWhereFiltroPadre($value)  {
        $this->val = $value;
    ...
    public function insertFiltroPadre($filtro) {
        $this->col = implode(",",$this->column);

        // echo $this->col;
        // echo $this->val;
        $sql = "INSERT INTO {$this->table} ({$this->pkey},{$this->col}) VALUE (NULL,'$filtro')";
        // echo $sql;
        $this->db->query($sql);
    }

    public function updateFiltroPadre($value,$filtro)  {
    ...
    public function deleteFiltroPadre($value)  {
        $this->id = $value;
