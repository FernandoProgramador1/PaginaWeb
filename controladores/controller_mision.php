...
class Misiones extends Conectar {
    private $table;
    private $view;
    private $id;
    private $lastid;
    ...
    public function getWhereMision($value)  {
        $this->val = $value;

        $sql = "SELECT * FROM {$this->view} WHERE IdMision='{$this->val}' ";
        // echo $sql;
        $result = $this->db->query($sql);
        while($row = $result->fetch_assoc())   {
            $this->field[] = $row;
    ...
    public function insertMision($desc) {
    ...
    public function updateMision($value,$desc)  {
    ...
    public function deleteMision($value)  {
        $this->id = $value;
